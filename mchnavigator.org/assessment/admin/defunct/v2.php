...
// ainsleytorres@berkeley.edu
// alyana_almenar@berkeley.edu
// christiane@berkeley.edu
...
$tab2 = array();
// $tab2[] = 'date, response, response_summary_knowledge, response_summary_skills, section, SA_Users::email, action';
$tab2[] = 'date, response, knowledge, skills, section, email, type';
...
foreach ( $tab2 as $key => $line ) {
    echo '<tr id="row_'.$key.'" class="datarow"><td>'.str_replace(', ', '</td><td>', $line).'</td><td data-csv="'.$line.'">';
    if( 0 != $key ){
		// echo '<select class="prepost" data-csv="'.$line.'"><option value="">-</option><option value="pre">pre</option><option value="post">post</option></select>';
		echo '<label><input type="radio" value="" name="prepost_'.$key.'" checked>skip</label>
	  <label><input type="radio" value="pre" name="prepost_'.$key.'">pre</label>
	  <label><input type="radio" value="post" name="prepost_'.$key.'">post</label>';
    }
    echo '</td></tr>'.PHP_EOL;
}
echo '</table>';

...
<head>
	...
	<title>Assesserella 3,000</title>
...
foreach ($emails as $email) {
	...
	$response = do_filemaker_request( build_request($email, 'SA_Responses'), 'array' );
	if ( empty($response['response'])) {
		...
		$user_version = $email . ' is 3.0';
		$tab1[] = $email.', 3.0, '.$response['response']['dataInfo']['foundCount'];
		$user_data = $response['response']['data'];
	}

...
// Version: 2025.10.21 - This is via php include only; it does not provide a public web endpoint.
session_start();

// define( "API_PATH", 'includes/FileMaker_DataAPI.php' );
define( "FM_URL", 'https://a812617.fmphost.com/fmi/data/v2/' ); // FM19
// define( "FM_URL", 'https://a0110806.fmphost.com/fmi/data/v2/' ); // FM24 demo


// This does all the FM DATA API work.
function get_filemaker_auth( $database ) {
	if( !$database){
		return null;
	}

	// credentials --> root doc
	// $auth = file_get_contents( API_PATH . '.auth' );
	// define( "USERPASS", base64_encode( $auth ));
	define( "USERPASS", base64_encode( 'mchlibrary.org:sTBD^@1PrrRR*F*hV5ZAW5CDBzER*Kp8PkTaCKp&7UPjd#qSDv7t3PsGpj&RmDf^' ));
	// mchlibrary.org:[km(Xur4nXXY)k=gJz%;kfM}CxFq=29#4*H*
	...
	if (!isset($_SESSION['FM_TOKEN']) || !isset($_SESSION['FM_TIMESTAMP']) || (time() - $_SESSION['FM_TIMESTAMP'] > 900)) {
		...
		if ( $json['messages'][0]['message'] === 'OK' ) {
			$_SESSION['FM_TOKEN'] = $json['response']['token'];
	...
	$_SESSION['LAST_ACTIVITY'] = time(); // update last activity time stamp
	define( "AUTH_TOKEN", 'Authorization: Bearer ' . $_SESSION['FM_TOKEN']);
}
...
function do_filemaker_request( $query_data, $format = 'array' ) {
	...
	$queryString    = isset( $query_data['queryString'] ) ? rawurlencode($query_data['queryString']) : null;
	...
	$script = isset( $query_data['script'] ) ? strip_tags(htmlspecialchars($query_data['script'])) : null;
